...
# Applied Statistics facts established by the current public notice even though the
# category-by-category campus-double-major workbook is not yet machine-ingested.
...
PHYSICS_ELECTIVE_2026 = CreditBucketRequirement(
    "second_physics_electives", "Physics Major Electives", target_credits=9.0,
    qualification_rule_id="physics_major_elective_2026", source=PHYSICS_REQUIREMENT_SOURCE,
)
PHYSICS_ELECTIVE_2026_CODES = frozenset({
    "PHY2103", "PHY2104", "PHY2106", "PHY3105", "PHY3108", "PHY3109", "PHY4101", "PHY4102", "PHY4107", "PHY4109",
    "PHY4113", "PHY4115", "PHY4116", "PHY4117", "PHY4205", "PHY4206", "PHY4207", "PHY4208",
})
...
EEE_ELECTIVE_2026_CODES = frozenset({
    "EEE2001", "EEE2112", "EEE3120", "EEE3150", "EEE3210", "EEE3220", "EEE3240", "EEE3310", "EEE3314", "EEE3350", "EEE3410",
    "EEE3430", "EEE3440", "EEE3450", "EEE3510", "EEE3511", "EEE3530", "EEE3535", "EEE3540", "EEE3543", "EEE3544", "EEE3545",
    "EEE3547", "EEE3548", "EEE4110", "EEE4120", "EEE4140", "EEE4240", "EEE4250", "EEE4260", "EEE4270", "EEE4280", "EEE4290",
    "EEE4320", "EEE4340", "EEE4350", "EEE4420", "EEE4430", "EEE4624",
})
EEE_ELECTIVE_2026 = CreditBucketRequirement("second_eee_electives", "EEE Major Electives", target_credits=6.0, qualification_rule_id="eee_major_elective_2018_plus", source=EEE_REQUIREMENT_SOURCE)
...
PHYSICS = SecondMajorCandidate(
    ...
    evidence_notes=("Current 2024+ Physics second-major rule: 27 required + 9 elective = 36 credits.", "Required course identities and current elective catalogue are published by the department."),
    unresolved_notes=("Future offering frequency and timetable geometry remain future-catalogue uncertainty.", "Recommended prerequisite text is not encoded as a hard prerequisite without a binding rule."),
)
...
ELECTRICAL_ELECTRONIC_ENGINEERING = SecondMajorCandidate(
    ...
    evidence_notes=("Current campus double-major table labels the active rule 2018+ and totals 36 credits.", "Structure: EEE2020 3 credits + eight fixed required courses 24 credits + one designated experiment 3 credits + published electives 6 credits.", "EEE2113 is explicitly marked as having no completion obligation in the current table."),
    unresolved_notes=("The table recommends engineering-foundation study for practical course access but does not make that recommendation a double-major graduation requirement.", "Future offering frequency, prerequisites, and timetable geometry remain downstream evidence questions."),
)
...
APPLIED_STATISTICS = SecondMajorCandidate(
    ...
    unresolved_notes=("The current category-by-category double-major spreadsheet could be identified but not parsed through the available web/connector interface, so exact Basic/Requisite/Elective minima are not encoded.", "Do not replace the missing category structure with a generic 36-credit bucket."),
)

MATHEMATICS = SecondMajorCandidate(
    candidate_id="mathematics", spec=SecondMajorSpec(status=SecondMajorStatus.UNRESOLVED, name="Mathematics"),
...
INDUSTRIAL_ENGINEERING = SecondMajorCandidate(
    candidate_id="industrial-engineering", spec=SecondMajorSpec(status=SecondMajorStatus.UNRESOLVED, name="Industrial Engineering"),
    evidence_notes=("The department currently publishes the entry-year-specific workbook 산업공학과_졸업요건표(2025.06.19).xlsx and explicitly instructs students to apply the row for their admission year.",),
    unresolved_notes=("The official attachment could be identified but not parsed through the available web/connector interface, so its current second-major category/course structure is not encoded.", "Courses absent from the table may require department-office confirmation, so no inference from the general IE curriculum is substituted for the workbook."),
)
COMPUTER_SCIENCE = SecondMajorCandidate(
    candidate_id="computer-science", spec=SecondMajorSpec(status=SecondMajorStatus.UNRESOLVED, name="Computer Science"),
    evidence_notes=("Current Software guidance states that a second Software major must satisfy its required + elective major credits, totaling 36 credits, and excludes major-exploration and advanced-only courses from second-major credit.", "The current School of Computing separately publishes a newer CAS-coded curriculum for recent entrants."),
    unresolved_notes=("The formal 2026 second-major identity/curriculum mapping between the older Software-major rules and the newer School of Computing CAS curriculum is not established well enough to encode a single requirement graph.", "Do not silently apply the 2021-23 SWE table to a 2026 candidate or treat every CAS course as second-major credit."),
)

SECOND_MAJOR_CANDIDATES_2026: tuple[SecondMajorCandidate, ...] = (MATHEMATICS, INDUSTRIAL_ENGINEERING, ELECTRICAL_ELECTRONIC_ENGINEERING, COMPUTER_SCIENCE, APPLIED_STATISTICS, PHYSICS)
